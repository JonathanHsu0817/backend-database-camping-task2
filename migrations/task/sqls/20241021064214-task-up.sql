...
VALUES 
('李燕容', 'lee2000@hexschooltest.io', 'USER', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('王小明', 'wXlTq@hexschooltest.io', 'USER', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('肌肉棒子', 'muscle@hexschooltest.io', 'USER', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('好野人', 'richman@hexschooltest.io', 'USER', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('Q太郎', 'starplatinum@hexschooltest.io', 'USER', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('透明人', 'opacity0@hexschooltest.io', 'USER', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
-- 2-1. 新增：在`CREDIT_PACKAGE` 資料表新增三筆資料，資料需求如下：
    ...
    INSERT INTO "CREDIT_PACKAGE" (name, price, credit_amount, created_at)
    VALUES 
    ('7 堂組合包方案', 1400, 7, CURRENT_TIMESTAMP),
    ('14 堂組合包方案', 2520, 14, CURRENT_TIMESTAMP),
    ('21 堂組合包方案', 4800, 21, CURRENT_TIMESTAMP);
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    -- 1. `王小明` 購買 `14 堂組合包方案`
    INSERT INTO "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid, created_at, purchase_at)
    SELECT 
        u.id,
        cp.id,
        cp.credit_amount,
...
-- 4-1. 新增：在`COURSE` 新增一門課程，資料需求如下：
    ...
    SELECT 
        ...
        s.id,  -- 重訓的 skill_id
        '重訓基礎課',
        '2024-11-25 14:00:00'::timestamp,
        '2024-11-25 16:00:00'::timestamp,
        10,
        'https://test-meeting.test.io',
        CURRENT_TIMESTAMP
    FROM "USER" u
    CROSS JOIN "SKILL" s
    WHERE u.email = 'lee2000@hexschooltest.io'  -- 使用 email 找到李燕容
    AND s.name = '重訓';  -- 找到重訓專長
...
-- 5-1. 新增：請在 `COURSE_BOOKING` 新增兩筆資料：
    -- 1. 第一筆：`王小明`預約 `李燕容` 的課程
        ...
        -- 3. 狀態`status` 設定為即將授課
        INSERT INTO "COURSE_BOOKING" (user_id, course_id, booking_at, status, created_at)
        SELECT 
            u.id,
            c.id,
            '2024-11-24 16:00:00'::timestamp,
            '即將授課',
            CURRENT_TIMESTAMP
        FROM "USER" u
        CROSS JOIN "COURSE" c
        JOIN "USER" coach_user ON c.user_id = coach_user.id
        WHERE u.name = '王小明'
        AND coach_user.email = 'lee2000@hexschooltest.io';
    -- 2. 新增： `好野人` 預約 `李燕容` 的課程
        -- 1. 預約人設為 `好野人`
        -- 2. 預約時間`booking_at` 設為2024-11-24 16:00:00
        -- 3. 狀態`status` 設定為即將授課
...
-- 5-2. 修改：`王小明`取消預約 `李燕容` 的課程，請在`COURSE_BOOKING`更新該筆預約資料：
    ...
    SET 
        cancelled_at = '2024-11-24 17:00:00'::timestamp,
        status = '課程已取消'
...
-- 5-4. 查詢：取得王小明所有的預約紀錄，包含取消預約的紀錄
    SELECT cb.*, c.name as course_name, u.name as coach_name
...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    UPDATE "COURSE_BOOKING"
    SET 
        join_at = '2024-11-25 14:01:59'::timestamp,
        status = '上課中'
    WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明')
    AND course_id = (
        SELECT c.id 
...
-- 顯示須包含以下欄位： 組合包方案名稱, 銷售數量
    ...
    LEFT JOIN "CREDIT_PURCHASE" p ON cp.id = p.credit_package_id
    WHERE purchase_at >= '2024-11-01 00:00:00'
    AND purchase_at < '2024-12-01 00:00:00'
...
-- 顯示須包含以下欄位： 預約會員人數
    ...
    AND created_at < '2024-12-01 00:00:00'
    AND status != '課程已取消';
